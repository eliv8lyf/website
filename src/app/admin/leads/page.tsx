            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/[0.07]">
                  {['Name', 'Email', 'Organisation', 'Service', 'Message', 'Date', 'Status', 'Actions'].map(h => (
                    <th key={h} className="px-6 py-3 text-left text-white/30 text-xs tracking-widest uppercase font-normal whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {leads.map(lead => (
                  <tr key={lead.id} className="border-b border-white/[0.04] hover:bg-white/[0.02] align-top">
                    <td className="px-6 py-4 text-white whitespace-nowrap">{lead.first_name} {lead.last_name}</td>
                    <td className="px-6 py-4 text-white/60">{lead.email}</td>
                    <td className="px-6 py-4 text-white/60">{lead.organisation}</td>
                    <td className="px-6 py-4 text-white/60 whitespace-nowrap">{lead.service}</td>
                    <td className="px-6 py-4 text-white/40 max-w-[200px]">
                      <span className="line-clamp-2 text-xs">{lead.message}</span>
                    </td>
                    <td className="px-6 py-4 text-white/30 text-xs whitespace-nowrap">{formatDate(lead.created_at)}</td>
