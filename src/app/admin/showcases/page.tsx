...
export default function ShowcasesPage() {
  ...
  async function handleSave() {
    ...
    if (editing) {
      await (supabase.from('showcases') as any).update(payload).eq('id', editing.id)
    } else {
      await (supabase.from('showcases') as any).insert(payload)
    }
  ...
  return (
    <div className="p-10">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl font-bold text-white" style={{ fontFamily: 'Syne, sans-serif' }}>AI in Action</h1>
        <button
          onClick={openNew}
      ...
      <div className="bg-[#111318] border border-white/[0.07]">
        {loading ? (
          ...
          <div className="px-6 py-10 text-white/30 text-sm">
            No showcases yet.{' '}
            <button onClick={openNew} className="text-[#c9a84c] hover:underline">Create the first one</button>.
          </div>
        ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-[#111318] border border-white/[0.07] w-full max-w-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-7 py-5 border-b border-white/[0.07]">
              ...
              <button onClick={closeForm} className="text-white/30 hover:text-white text-lg transition-colors">✕</button>
            </div>

            <div className="px-7 py-6 flex flex-col gap-5">
              <div>
                <label className={labelCls}>Title *</label>
                <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} className={inputCls} placeholder="Showcase title" />
              </div>
              <div>
                <label className={labelCls}>Description *</label>
                <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={3} className={`${inputCls} resize-none`} placeholder="Short description" />
              </div>
              <div>
                <label className={labelCls}>Category *</label>
                <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value as Showcase['category'] }))} className={inputCls}>
              ...
              <div>
                <label className={labelCls}>Media URL (image or video)</label>
                <input value={form.media_url} onChange={e => setForm(f => ({ ...f, media_url: e.target.value }))} className={inputCls} placeholder="https://…" />
              </div>
              <div>
                <label className={labelCls}>Media Type</label>
                <select value={form.media_type} onChange={e => setForm(f => ({ ...f, media_type: e.target.value as Showcase['media_type'] }))} className={inputCls}>
              ...
              <div>
                ...
                <input value={form.tags} onChange={e => setForm(f => ({ ...f, tags: e.target.value }))} className={inputCls} placeholder="e.g. LLM, Automation, Healthcare" />
              </div>
              <div>
              ...
              <label className="flex items-center gap-3">
                <input type="checkbox" checked={form.published} onChange={e => setForm(f => ({ ...f, published: e.target.checked }))} className="w-4 h-4 accent-[#c9a84c]" />
                <span className="text-white/60 text-sm">Published (visible on website)</span>
              </label>
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={closeForm} className="px-6 py-2.5 bg-white/[0.06] text-white/60 text-sm hover:bg-white/[0.1] hover:text-white transition-colors">
                  Cancel
                </button>
              </div>
            </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-[#111318] border border-white/[0.07] w-full max-w-sm p-8">
            <h2 className="text-lg font-bold text-white mb-3" style={{ fontFamily: 'Syne, sans-serif' }}>Delete showcase?</h2>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => setDeleteId(null)} className="px-6 py-2.5 bg-white/[0.06] text-white/60 text-sm hover:bg-white/[0.1] hover:text-white transition-colors">Cancel</button>
            </div>
          </div>
        </div>
      )}
